...
class ShowOrganizersScreen extends StatelessWidget {
  final Event event;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          IconButton(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => AddOrganizerScreen(event: event,),
                )
              );
            },
      ...
      body: FutureBuilder(
        future: EventsController.getOrganizers(event.id),
        builder: (context, AsyncSnapshot snapshot) {
          if(snapshot.connectionState == ConnectionState.done) {
            return ListView.builder(
              itemCount: snapshot.data.length,
              itemBuilder: (context, index) {
                return _organizer(snapshot.data[index]);
              },
            );
          }
